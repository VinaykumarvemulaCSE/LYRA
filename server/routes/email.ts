...
router.post("/contact", async (req: Request, res: Response) => {
  try {
    ...
    if (!adminEmail) return res.status(500).json({ message: "Admin email not configured on server." });
    ...
    await sendStoreEmail(adminEmail, `[Contact] ${subject || "Message from " + firstName}`, html);
...
router.post("/test", async (req: Request, res: Response) => {
  try {
    ...
    if (!recipient) return res.status(400).json({ message: "No recipient email provided" });
    await sendStoreEmail(recipient, "LYRA Backend Test Email", "<p>✅ Email system is working correctly from Render.</p>");
